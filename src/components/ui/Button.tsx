...
export default function Button({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex min-h-11 items-center whitespace-nowrap justify-center rounded-md px-10 py-4 text-sm font-semibold transition ${variantClasses[variant]} ${className}`}
      {...props}
    >
      {children}
    </Link>
